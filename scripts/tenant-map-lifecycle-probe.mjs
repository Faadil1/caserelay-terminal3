...
try {
  ...
  try {
    ...
    try {
      ...
      try {
        const updateResult = await tenantControlClient.maps.update(tail, {
          readers: "all",
        });

        console.log("MAP_UPDATE_PASS");
        console.log(
          "MAP_UPDATE_RESULT_TYPE:",
          Array.isArray(updateResult) ? "array" : typeof updateResult
        );
      } catch (updateError) {
        console.log("MAP_UPDATE_FAILED");
        console.log(
          "MAP_UPDATE_ERROR:",
          updateError instanceof Error
            ? updateError.message.slice(0, 500)
            : String(updateError).slice(0, 500)
        );
      }
    } catch (mapError) {
